 ...
 * Copyright (c) 2011 Allan Ma (/web/20131229054203/http://codecanyon.net/user/webtako)
 * Version: 1.5.7 (04/04/2011)
 */
;(function($) {
	$.fn.wtRotator = function(params) {
		...
		function Rotator($obj, opts) {
			...
			var bpMouseover = 	opts.bpanel_mouseover;
			...
			// tom added 7/7/11
			var liWidthSum=0;
			$(".marquee-rotator .thumbnails ul li").each( function(){ liWidthSum += $(this).outerWidth(true);});
			$(".marquee-rotator .thumbnails").css({width:liWidthSum});
			//alert(liWidthSum);
			
			//init rotator
			this.init = function() {
				$rotator = 	$obj.find(".marquee-rotator");
				$screen = 	$rotator.find("div.screen");
				$cpanel 	= $rotator.find("div.c-panel");
				$buttonPanel= $rotator.find("div.buttons");
			...
			var initCPanel = function() {	
				if (displayThumbs || displayDBtns || displayPlayBtn) {
					var maxWidth = screenWidth - (margin);
...
//						var cpHeight = $cpanel.outerHeight(true);
						...
						if (bpMouseover) {
							$rotator.hover(displayBPanel, hideBPanel);
						}
					}
					if(cpMouseover)
						$cpanel.css("display", "none").click(preventDefault);
					else
						$cpanel.css("display", "block").click(preventDefault);

					if(bpMouseover)
						$buttonPanel.css("display", "none").click(preventDefault);
					else
						$buttonPanel.css("display", "block").click(preventDefault);
				}
			}
...
//				$cpanel.data({});
			...
			var initTooltip = function() {
				...
				if (tipType == "text") {
					$tooltip.append("<div class='tt-txt'/>");
					upClass = "txt-up";
					downClass = "txt-down";
			...
			var showImgTooltip = function(e) {
				var $img = $items[$(this).index()].data("tt-img");
				if ($img != undefined) {
					...
					if ($img[0].complete || $img[0].readyState == "complete") {	
						...
						$tooltip.css({top:offset.top + yOffset, left:offset.left + (($(this).outerWidth() - $tooltip.outerWidth())/2)})
								.stop(true, true).delay(TOOLTIP_DELAY).fadeIn(100);
					}
				}
			}
...
//				$cpanel.stop(true).animate({top:$cpanel.data("offset"), opacity:0}, ANIMATE_SPEED);
			...
			var displayBPanel = function() {
				$buttonPanel.show();
			}

			//hide button panel
			var hideBPanel = function() {
				$buttonPanel.hide();
			}
			...
			var expandText = function(text, data, props1, props2) {
				$innerText.html("");
				$textBox.stop(true).css({display:"block", top:data.y, left:data.x, "margin-top":0, "margin-left":0}).css(props1).animate(props2, ANIMATE_SPEED, 
					function () {  
						$innerText.html(text);
					});  
			}
